...
export default function BookBus({ bus }) {
  ...
  // If bus doesn't exist or has no seats
  if (!bus || bus.seats <= 0) {
    return (
      <div className="py-20 flex flex-col items-center justify-center">
        <h1 className="text-2xl font-bold text-gray-800 mb-4">
          {!bus ? 'Bus Not Found' : 'No Seats Available'}
        </h1>
        ...
        <Link href="/buses" className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-medium transition">
          Back to Buses
        </Link>
      </div>
    );
  ...
  if (status === 'unauthenticated') {
    router.push(`/auth/login?callbackUrl=${encodeURIComponent(`/buses/book/${bus._id}`)}`);
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Please log in to book this bus</p>
  ...
  const classMultiplier = selectedClass === 'Standard' ? 1 : selectedClass === 'Business' ? 1.5 : 2;
  ...
  const handleSubmit = async (e) => {
    ...
    if (passengerCount > bus.seats) {
      setError(`Only ${bus.seats} seats available for this bus`);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const updateSeatsResponse = await fetch(`/api/buses/${bus._id}/seats`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          seats: bus.seats - passengerCount
        })
      });
  ...
  return (
    <>
      <Head>
        <title>Book Bus: {bus.from} to {bus.to} | TravelEase</title>
      </Head>

      <div className="bg-gray-50 py-8 px-4">
        <div className="max-w-5xl mx-auto">
          <Link href={`/buses/${bus._id}`} className="inline-flex items-center text-indigo-600 hover:text-indigo-800 mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-800 mb-6">Book Your Bus Ticket</h1>
          ...
          {success ? (
            <div className="bg-green-50 border border-green-200 text-green-700 px-6 py-8 rounded-lg text-center">
              ...
              <p className="mb-4">Your bus ticket has been booked successfully. Redirecting to your bookings...</p>
              ...
              <div className="lg:col-span-1">
                <div className="bg-white rounded-lg shadow-md overflow-hidden">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-600 mb-4">{bus.busCompany} • Bus {bus.busNumber}</p>
                    ...
                    <div className="border-t border-gray-200 pt-4 mb-4">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-800 font-medium">{bus.duration}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full text-black px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                        >
                          {[...Array(Math.min(bus.seats, 10))].map((_, i) => (
